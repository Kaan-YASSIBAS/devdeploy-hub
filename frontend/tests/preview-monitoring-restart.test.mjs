...
const previewPath = source("../src/lib/preview-path.ts");
...
test("GitOps create modal supports safe optional preview paths", () => {
  ...
  assert.match(createModal, /import \{ normalizePreviewPath \} from "@\/lib\/preview-path"/);
  assert.match(previewPath, /export function normalizePreviewPath\(value: string\)/);
  assert.match(previewPath, /if \(!candidate\) return "\/"/);
  assert.match(previewPath, /return `\/\$\{normalized\}`/);
  ...
  assert.match(previewPath, /startsWith\("http:\/\/"\)/);
  assert.match(previewPath, /startsWith\("https:\/\/"\)/);
  assert.match(previewPath, /startsWith\("\/\/"\)/);
  assert.match(previewPath, /segment === "\." \|\| segment === "\.\."/);
  assert.match(previewPath, /candidate\.includes\("\\\\"\)/);
