...
- Managed app destroy is implemented through explicit product records and scoped cleanup. Broader prune/delete policy remains constrained because the Root Application currently uses `prune=false`.
...
- **Phase 2J.24 - Safe Backend App Preview Proxy:** add owner-scoped, GET-only backend preview routes that resolve the target exclusively from an active `DeploymentRecord` and its trusted runtime Service metadata. The backend uses the configured workload Kubernetes client and Kubernetes API Service proxy; clients cannot supply a target URL, host, port, namespace, or Service name. An authenticated access check creates a two-minute, deployment-scoped HttpOnly preview session so a new browser tab can load the backend-owned relative preview URL without placing the user's JWT in a URL. Preview paths reject external URLs, network-path references, traversal, encoded target tricks, control characters, and query parameters. The proxy does not forward browser authorization, cookies, host, or arbitrary headers; redirects are not followed, upstream cookies and hop-by-hop headers are not returned, requests use a bounded five-second read timeout, and responses are limited to 2 MiB and an explicit content-type allowlist. Proxied content receives a sandbox CSP and other browser hardening headers. This is a local-first preview for basic path-relative static content, not production ingress: root-relative assets, queries, WebSockets, streaming, mutation methods, and external redirects are not supported. No ClusterIP or Kubernetes API URL is exposed, and no Kubernetes, GitOps, or Argo CD mutation is added.
- **Phase 2J.25 - Safe Application Destroy Flow:** add owner-scoped `POST /api/v1/deployment-records/{deployment_id}/destroy` for active managed records. The backend removes only the server-derived app folder under `gitops/workloads/devdeploy-apps/apps/<app-name>`, removes the root Kustomization entry, commits and pushes the scoped GitOps change, then archives the same `DeploymentRecord` as `destroyed` while leaving the linked `ServiceDefinition` intact. Because the Root Application still uses `prune=false`, runtime cleanup is reported separately and remains exact, label-verified, and limited to the owned Deployment and Service in the managed namespace through the Kubernetes Python client. After the destroy commit is pushed, DevDeploy requests a scoped normal refresh and normal sync on the configured root Argo CD Application, with `prune=false` and no force options. Pre-cleanup readiness requires the root Application to observe the exact destroy revision and report a successful normal sync operation for that revision; `OutOfSync` is expected before cleanup while prune is disabled because the removed Git resources still exist in runtime. DevDeploy then deletes the exact owned runtime resources, verifies stable absence, and treats final `Synced` as a post-cleanup reconciliation signal that may lag briefly. If Argo reconciliation cannot be requested, observation times out, status is unavailable, or resources reappear during stabilization, the record stays destroyed/archived and runtime cleanup is reported as pending or failed for safe retry. The flow does not enable global prune, force sync Argo CD, accept client-supplied paths or Kubernetes targets, use kubectl, delete broad selectors, or mutate unrelated Argo Applications or management-cluster resources.
- **Phase 2J.26 - Destroyed Deployment Recover / Redeploy:** add owner-scoped recovery for archived `DeploymentRecord` rows whose `desired_state=destroyed`. Recovery reuses the same record and linked `ServiceDefinition`, regenerates deterministic GitOps manifests through a strict restore mode, rejects mismatched existing app folders or duplicate root entries, commits and pushes the scoped GitOps change, requests normal Argo CD refresh/sync with `prune=false`, waits for the exact recovery revision and successful Argo operation, verifies exact Deployment and Service readiness with ownership labels, and only then clears the current archive state. Partial Git, Argo, or runtime failures leave the record destroyed/archived for safe retry without creating duplicate product records or root Kustomization entries.
- **Phase 2J.27a - Observability Stack Discovery, Bootstrap, and Backend Health Foundation:** document the current and historical observability implementation, select workload-local collection with backend-mediated read APIs, and add a sanitized observability status foundation for Kubernetes, Prometheus, Loki, and optional Grafana. This phase does not claim live stack installation or implement full Monitoring, Logs, or Dashboard analytics.
- **Phase 2J.27b - Automated Observability Bootstrap and Cross-Cluster Transport:** add explicit launcher modes to bootstrap or verify the workload observability stack in `devdeploy-workload/monitoring`, add Helm values for kube-prometheus-stack, Loki, Grafana Alloy, and Grafana datasource provisioning, generate Grafana credentials into a Kubernetes Secret, and configure the management backend for Kubernetes API Service proxy reads through a narrow workload service account. The backend transport uses fixed server-controlled namespace/service/port/path settings, never exposes ClusterIP URLs to the frontend, and does not use `kubectl`, shell execution, or client-controlled proxy targets. Live installation remains an explicit launcher action and is not run by default.
- **Phase 2J.27c (next) - Observability Smoke and Product Integration:** run the explicit workload observability bootstrap in the local environment, verify `/api/v1/observability/status`, then connect product Monitoring/Logs/Dashboard views to owner-scoped workload-domain APIs without arbitrary PromQL or LogQL from the browser.
- **Later - Runtime Resource Import and Adoption Design:** define explicit review, ownership, desired-manifest generation, and conflict handling before adding any import or adoption action.
...
- Continue evaluating a future prune/delete policy before claiming Argo CD pruning alone deletes runtime resources.
