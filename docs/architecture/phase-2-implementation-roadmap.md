...
- **Phase 2J.12 (next) - Deployment Runtime Status Enrichment:** reconcile `DeploymentRecord` lifecycle summaries with the existing read-only Argo CD and workload status model without turning GitOps discovery into the primary product list.
