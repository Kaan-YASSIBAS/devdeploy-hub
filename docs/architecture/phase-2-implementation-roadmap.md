...
- **Phase 2J.13 (next) - Untracked Runtime Resource Discovery and Import Design:** define an explicit recovery/import workflow for Kubernetes resources that have no matching product record. DB-less Deployments and Services remain excluded from the main product lists until that design is reviewed.
