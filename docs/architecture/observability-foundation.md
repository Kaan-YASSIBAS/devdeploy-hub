...
The launcher creates one narrow workload runtime identity for backend reads. In-cluster, its kubeconfig is mounted from a management-cluster Secret and is shared by general read-only runtime and observability readers. For local uvicorn development, the launcher writes the same identity to `.devdeploy/local/kubeconfig/observability-workload-kubeconfig.yaml` and configures the observability setting with that portable path. The normal workload kubeconfig remains separate and is not rewritten by observability bootstrap; operations such as exact, owner-scoped runtime cleanup must use an explicitly configured credential with only the permissions that operation requires. The host-local read-only representation derives its server and CA data from the selected source kubeconfig/context; the in-cluster representation keeps the Docker-network endpoint. The read-only identity has namespace and node discovery plus read-only Deployment, Service, and Pod inventory across workload-cluster namespaces. GET-only Service proxy access is limited to configured observability Services in `monitoring` and owner-scoped app previews in `devdeploy-apps`; it is not granted cluster-wide. It has no workload write verbs and cannot read Secrets or ConfigMaps.
