...
In Docker Compose, `KUBERNETES_IN_CLUSTER=false` and the Prometheus/Loki URLs may point at localhost. In the multi-cluster local runtime, the management backend uses Kubernetes service-proxy mode. The launcher writes one narrow read-only workload kubeconfig under `.devdeploy/local/kubeconfig/observability-workload-kubeconfig.yaml` and configures `DEVDEPLOY_OBSERVABILITY_WORKLOAD_KUBECONFIG` to use it. General read-only runtime/status readers use this same managed identity. `DEVDEPLOY_WORKLOAD_KUBECONFIG` remains separate and is never overwritten by observability bootstrap because scoped runtime cleanup requires a distinct explicitly configured credential. The local read-only file uses the host-reachable workload API endpoint from the selected source kubeconfig/context, while the in-cluster Secret keeps the Docker-network endpoint. If those services are not reachable from the backend, observability endpoints return `503` instead of crashing startup.
